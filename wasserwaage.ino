...
float accX = 0;
float accY = 0; // accY will now likely hold a relatively static value (e.g., ~-1g) when held upright
float accZ = 0; // accZ will now be used for vertical movement as it tracks pitch in this orientation
...
void setup() {
    ...
    // Uncomment the line below to enable serial output for debugging accelerometer values
    // Serial.begin(115200);
}
...
void loop() {
    ...
    // The original comments on Y and Z axis behavior were incorrect for this specific physical orientation.
    M5.IMU.getAccelData(&accX, &accY, &accZ);

    // --- Debugging (Uncomment the Serial.printf lines to see raw sensor data and mapped coordinates) ---
    ...
    // --- Debugging (Uncomment to see mapped coordinates) ---
    // if (Serial.availableForWrite()) {
